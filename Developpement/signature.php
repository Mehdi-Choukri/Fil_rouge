    ...
    if(isset($_POST['signaturesubmit'])){ 
      ...
      $SignedByUser = $user->data()->EMAIL;
        ...
        if(Input::exists())
        {
            ...
            $signedverify1 = $allDocs->SIGNED_BY1;

            $signedverify2 = $allDocs->SIGNED_BY2;
            //roleuser = 2 pour le premier signataire et 3 pour le dernier 

            $newcptSigned = $cpt_SIGNED;
            if($roleUser == 2)
            {
            ...
            if ( $cpt_SIGNED<= 2)
            {
                if($signedverify1 == '' &&  $roleUser == 2  )
                {
                    $newcptSigned = $cpt_SIGNED +1;
                }
                if($signedverify2  == '' && $roleUser == 3)
                {
                    $newcptSigned = $cpt_SIGNED +1;
                }
...
<body>
    <div class="container signature_container">
        <div class="row">
